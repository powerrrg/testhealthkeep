	...
	<div class="iHold">
		...
		<div class="iBoxHeadingColoured clearfix margin0">
			<div class="iBoxHeadingColouredHeading iBoxHeading_<?php echo $typeArray["color"]; ?> clearfix">
				<h3>Add your daily number of minutes you exercised</h3>
			</div>
		</div>
		
		<form action="<?php echo WEB_URL; ?>act/measurement/add_exercise.php" id="measurement" method="post" class="addEventForm">
			<div class="addEventFormItem clearfix">
				<h4>Please enter the number of minutes</h4>
				<div class="addEventFormInputs">
				<input type="text" id="exercise" name="exercise" class="numeric" placeholder="minutes" maxlength="4" /> 
				</div>
			</div>
			<div class="addEventFormItem clearfix">
		...
		$onload.="$('#exercise').focus();
		$('#measurement').submit(function(){
			if($('#exercise').val()==''){
				alert('You need to set a value');
				$('#exercise').focus();
				return false;
			}else if($('#exercise').val()<1 || $('#exercise').val()>1440){
				alert('The number of minutes you set is invalid!');
				$('#exercise').focus();
				return false;
