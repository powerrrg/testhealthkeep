	...
	<div class="iHold">
		...
		<div class="iBoxHeadingColoured clearfix margin0">
			<div class="iBoxHeadingColouredHeading iBoxHeading_<?php echo $typeArray["color"]; ?> clearfix">
				<h3><img src="<?php echo WEB_URL; ?>inc/img/v1/topic/white/<?php echo $typeArray["icon"]; ?>" /> Add Symptoms</h3>
			</div>
		</div>
		
		<form action="<?php echo WEB_URL; ?>act/timeline/add_symptoms.php" id="symptom" method="post" class="addEventForm">
			<div class="addEventFormItem clearfix">
				<?php
				$cantFindWhat="symptom";
			...
			<div class="addEventFormItem clearfix">
				<h4>When did the symptoms start?</h4>
				<div class="addEventFormInputs">
				<?php
			...
			<div class="addEventFormItem clearfix">
				<h4>Are this symptoms still active?</h4>
				<div class="addEventFormInputs">
		...
		<?php
		$needTokenInput=1;
		$onload.="$('#topic').tokenInput('".WEB_URL."act/ajax/autoCompleteTopic.php?type=s&cantFind=1', { hintText: 'Type the symptom', noResultsText: 'No symptom with that name', searchingText: 'Searching...',tokenLimit: 10,minChars: 2,searchDelay: 200,preventDuplicates: true, theme:'long',onAdd:function(item){ if(item.id==0){ $('#topic').tokenInput('clear'); $('#cantFind').click(); } } });";
		
		$onload.="$('#symptom').submit(function(){
			if($('#topic').val()==''){
				alert('You need to set a symptom');
				$('#topic').focus();
				return false;
